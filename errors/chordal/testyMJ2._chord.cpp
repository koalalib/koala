...
#include "../../main.hpp"
    ...
    {   cout <<"Undirected (g6 or matrix format):\n";
        MyGraph g;
        MyGraph::PVertex tabv[100]={0,0,0,0,0,0,0};
        int n=11;

//        g.makeAdjMatrix();

        int u=1, v=3, w=6;
        for(int i=0;i<n;i++) tabv[i]=g.addVert();
        for(int i=0;i<n;i++) for(int j=i+1;j<n;j++)
            if (! (((i==u) && (j==v)) || ((i==u) && (j==w)))) g.addEdge(tabv[i],tabv[j]);
    ...
    g.clear();
        ...
        g.neg(EdUndir);IO::writeGraphText(g, cout, IO::RG_VertexLists );
