...
void readG6(Graph &graph, const char *str_graph) {
	int mask = 0x20;
	...
	for(int i=1; i<vert_no; i++) {
		...
		while(j<i) {
			if(!mask) {
				++ch;
				if(!( *ch ))
					throw KoalaG6Exception(2);
				if(*ch<LIMIT_LO || *ch>LIMIT_HI)
					throw KoalaG6Exception(1);
				*ch -= LIMIT_LO;
				mask = 0x20;
			}

			if( *ch & mask )
				graph.addEdge(vert_vect[i], vert_vect[j]);
...
//	int mask = 0;
	...
	//write order of the graph
	if(order<LIMIT_HI) {
		str_graph.push_back(order+LIMIT_LO);
	} else {
...
int writeG6(const Graph &graph, char *str_graph, int len_str) {
	...
	while(tmp_len<len_str) {
		*str_graph = (char)(((*ch_lo)>>2)+LIMIT_LO);
		str_graph++;
		tmp_order += 6;
		if(tmp_order>=order2 || tmp_len>len_str)
			break;
		ch_hi = ch_lo;
		ch_lo++;
