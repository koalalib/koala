...
std::string GraphMLKeyVal::print(bool isUsr) {
	...
	if( isUsr ) {
		...
		case GraphMLKeyVal::Long:
			sprintf(tmp_ch, "%lld", pUsr.longVal); //jendrek: it has to be changed
			return tmp_ch;
		...
		case GraphMLKeyVal::Int:
			...
			return tmp_ch;
		case GraphMLKeyVal::Long:
			sprintf(tmp_ch, "%lld", pDef.longVal); //jendrek: it has to be changed
			return tmp_ch;
		case GraphMLKeyVal::Float:
		case GraphMLKeyVal::Double:
...
bool GraphMLKeyVal::setDef(const char *val) {
	...
	switch(type) {
		...
		case GraphMLKeyVal::Long:
			...
			break;
		case GraphMLKeyVal::Float: case GraphMLKeyVal::Double: {
			char *endP;
...
KeysHolderBase::KeysHolderBase(const KeysHolderBase &khBase) {
	...
	this->defs = khBase.defs; //copy pointer
}
...
KeysHolderRead::~KeysHolderRead() {
	this->defs = NULL; //defs is pointer to a vector that is still in use
}

bool KeysHolderRead::valid(int id) {
	if(id<=0 || id > KeysHolderBase::getKeysNo())
		return false;
	GraphMLKeyVal::ForKey forKey = KeysHolderBase::getKeyFor(id);
	switch(type) {
		case 0: return true;
		case 1: //graph
			if(forKey==GraphMLKeyVal::All
			|| forKey==GraphMLKeyVal::Graph)
				return true;
			return false;
		case 2: //vertex
			if(forKey==GraphMLKeyVal::All
			|| forKey==GraphMLKeyVal::Node)
				return true;
			return false;
		case 3: //edge
			if(forKey==GraphMLKeyVal::All
			|| forKey==GraphMLKeyVal::Edge)
				return true;
			return false;
	}
	return false;
...
int KeysHolderWrite::getIdCr(const char *keyName) {
	int id = KeysHolderBase::getId(keyName);
	if(!id) {
		id = KeysHolderBase::createKey(keyName);
	} else
		if(!valid(id))
			return 0;
	return id;
...
bool GraphML::readGraph(Graph &graph, TiXmlElement *xmlGraph) {
	...
	bool isDirected = true; //checking if graph is directed
	{
		const char *edgeDef = xmlGraph->Attribute("edgedefault");
		if( !strcmp(edgeDef, "undirected") )
			isDirected = false;
	}

	std::map<std::string, typename Graph::Vertex*> verts; //map: id(xml)->vert
...
bool GraphML::readGraph(Graph &graph, TiXmlElement *xmlGraph, InfoVertex infoVert,
	InfoEdge infoEdge)
{
	if(!xmlGraph)
		return false;
	...
	while(xmlEdge) {
		...
		const char *target = xmlEdge->Attribute("target");
		if( !strcmp(source,target) ) {
			graph.addLoop( verts[source], infoEdge(&khRead) );
		} else {
			bool edgeDirect = isDirected;
			const char *isEdgeDir = xmlEdge->Attribute("directed");
			if(isEdgeDir) {
			...
			if(edgeDirect)
				...
				graph.addEdge(verts[source], verts[target], infoEdge(&khRead));
		}
		xmlEdge = xmlEdge->NextSiblingElement("edge");
	}
	return true;
...
template<class Graph, class InfoVertex, class InfoEdge>
bool GraphML::readGraph(Graph &graph, const char *graphName, InfoVertex infoVert,
	InfoEdge infoEdge)
{
	if(!doc)
		return false;
...
template<class Graph, class InfoVertex, class InfoEdge>
bool GraphML::readGraph(Graph &graph, int graphNr, InfoVertex infoVert,
	InfoEdge infoEdge)
{
	if(!doc)
		return false;
...
bool GraphML::writeGraph(const Graph &graph, const char *graphName) {
	...
	while(edge) {
		...
		std::pair<typename Graph::Vertex*,typename Graph::Vertex*> verts
				= graph.getEdgeEnds(edge);
...
template<class Graph, class InfoVertex, class InfoEdge>
bool GraphML::writeGraph(const Graph &graph, const char *graphName, InfoVertex infoVert,
		InfoEdge infoEdge)
{
	if(!graphName || !*graphName)
		return false;
	...
	while(vert) {
		...
		for(int i=1; i<=khWrite.getKeysNo(); i++) {
			if( khWrite.modified(i) ) {
				...
				xmlKey->LinkEndChild( new TiXmlText( khWrite.print(i, true).c_str() ) );
			}
		}
...
bool GraphML::createInitial() {
	...
	xmlGraphs->SetAttribute("xsi:schemaLocation", "http://graphml.graphdrawing.org/xmlns "
...
bool GraphML::readKeys() {
	...
	//read keys and its default values
	...
	while(xmlKey) {
		...
		if(!keyFor)
			continue;
		else if(!strcmp(keyFor, "all"))
			forKey = GraphMLKeyVal::All;
		else if(!strcmp(keyFor, "node"))
			forKey = GraphMLKeyVal::Node;
		else if(!strcmp(keyFor, "edge"))
			forKey = GraphMLKeyVal::Edge;
		else if(!strcmp(keyFor, "graph"))
			forKey = GraphMLKeyVal::Graph;
		else
			continue;
		...
		if(!strcmp(keyType,"boolean"))
			typeKey = GraphMLKeyVal::Bool;
		else if(!strcmp(keyType,"int"))
			typeKey = GraphMLKeyVal::Int;
		else if(!strcmp(keyType,"long"))
			typeKey = GraphMLKeyVal::Long;
		else if(!strcmp(keyType,"float"))
			typeKey = GraphMLKeyVal::Float;
		else if(!strcmp(keyType,"double"))
			typeKey = GraphMLKeyVal::Double;
		else if(!strcmp(keyType,"string"))
			typeKey = GraphMLKeyVal::String;

		keysHolder.setKeyType(id, typeKey);

		TiXmlElement *xmlDefault = xmlKey->FirstChildElement("default");
		if(xmlDefault)
			keysHolder.setValDef(id, xmlDefault->GetText());
...
void GraphML::writeKeys() {
	...
	for(; iter!=keysHolder.defs->end(); ++iter) {
		if( iter->count==keysHolder.count )
			continue;

		if(xmlFirst) {
			xmlFirst = xmlGraphs->InsertBeforeChild(xmlFirst, *tmpElem);
		} else {
			xmlFirst = xmlGraphs->InsertEndChild(*tmpElem);
		}
		...
		if(iter->is_def) {
			...
			switch( iter->type ) {
			case GraphMLKeyVal::Bool:
				xmlDef->LinkEndChild(new TiXmlText(iter->pDef.intVal ? "1" : "0"));
				break;
			case GraphMLKeyVal::Int:
...
bool GraphML::writeGraphParam(TiXmlElement *xmlGraph, KeysHolderGraph &kHolderG) {
	...
	for(int i=1; i<=kHolderG.getKeysNo(); ++i) { //we have to search for new values
		if( !kHolderG.modified(i) )  //if value was not changed
			continue;
		int j=1;
		...
		if( j > this->keysHolder.getKeysNo() ) { //add key ...
			...
			&& this->keysHolder.getKeyFor(j)!=GraphMLKeyVal::Graph)
				continue; //if key is not for graph parameters
		}
		TiXmlElement *xmlKey = new TiXmlElement("data");
