...
namespace Koala
{
        ...
        public SubgraphBase, public ConstGraphMethods< Subgraph< Graph, VChooser, EChooser> >
    {
      ...
      protected:
        template <class T> static bool isEdgeTypeChooser( const T &x, Koala::EdgeDirection &val ) { return false; }
        static bool isEdgeTypeChooser( const EdgeTypeChooser &x, Koala::EdgeDirection &val );

        template <class T> static bool isBoolChooser( const T &x, bool &val ) { return false; }
    ...
    template< class Graph > class UndirView: public SubgraphBase, public ConstGraphMethods< UndirView< Graph> >
    {
      public:
        ...
        static EdgeType allowedEdgeTypes() { return (((~EdLoop)&ParentGrType::allowedEdgeTypes()) ? Undirected :0 )
    ...
    template< class Graph > class RevView: public SubgraphBase, public ConstGraphMethods< RevView< Graph> >
    {
      public:
        ...
        PEdge getEdgeNext( PEdge e, EdgeDirection mask = EdAll ) const { return up().getEdgeNext( e,(mask) ); }
        PEdge getEdgePrev( PEdge e, EdgeDirection mask = EdAll ) const { return up().getEdgePrev( e,(mask) ); }

        //          nie usuwac komentarza - waham sie co do wersji
        //        { return up().getEdgeNext(v,e,transl(mask)); }
        PEdge getEdgeNext( PVertex v, PEdge e, EdgeDirection mask = EdAll ) const { return getNext( v,e,transl( mask ) ); }
        
        //          nie usuwac komentarza - waham sie co do wersji
        //        { return up().getEdgePrev(v,e,transl(mask)); }
        PEdge getEdgePrev( PVertex v, PEdge e, EdgeDirection mask = EdAll ) const { return getPrev( v,e,transl( mask )); }
