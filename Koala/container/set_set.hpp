...
template< typename Element > inline
    Set< Element > operator^( const Set< Element > &s1, const Set< Element > &s2 ) {
        std::vector<Element> res;
        set_symmetric_difference( s1.begin(),s1.end(),s2.begin(),s2.end(),back_inserter(res) );
        return res;
    }
...
template< typename Element >
    Element Set< Element >::next(const Element& a)
    {
...
template< typename Element >
    Element Set< Element >::prev(const Element& a)
    {
        typename std::set<Element>::iterator i=this->find(a);
