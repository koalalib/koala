	...
	edgeToList( g.getEdgeNo(EdDirIn|EdDirOut|EdUndir) )
{
	...
	typedef typename Graph::PEdge Edge;
	const EdgeDirection Mask = EdDirIn|EdDirOut|EdUndir;
	tabVert = Koala::k__cast(tabVert, malloc( sizeof(*tabVert)*g.getVertNo() ));
	colorToList = (int*) malloc(sizeof(*colorToList)*(maxCol+1));
	listEdgeCol = Koala::k__cast(listEdgeCol,
		malloc( sizeof(*listEdgeCol)*(g.getEdgeNo(Mask)+1) ));
	maxColor = -1;
...
//--------------------begin of MaxMatchColoring-------------------------
//we assume that result (edges - 'out') are in the same order as input (edges - 'beg', 'end')
template <class DefaultStructs>
template<typename Graph, typename EInIter, typename EOutIter>
int MaxMatchColoringPar<DefaultStructs>::getMatching(const Graph &graph,
	EInIter beg, EInIter end, EOutIter out)
{
	typedef typename Graph::PVertex Vert;
	typedef typename Graph::PEdge Edge;
	Set<Vert> vertSet;
	...
	for(;beg!=end; ++beg) {
		...
		Vert v1 = graph.getEdgeEnd1(ee);
		Vert v2 = graph.getEdgeEnd2(ee);
		if(vertSet.isElement(v1)||vertSet.isElement(v2))
			continue;
		*(out++) = ee;
		vertSet.add(v1);
		vertSet.add(v2);
...
template <class DefaultStructs>
template<typename Graph, typename ColorMap, typename EInIter>
int MaxMatchColoringPar<DefaultStructs>::color(const Graph &graph,
	ColorMap &colors, EInIter beg, EInIter end)
{
	typedef typename Graph::PEdge Edge;
	const EdgeDirection Mask = EdDirIn|EdDirOut|EdUndir;
	Edge LOCALARRAY(edges, graph.getEdgeNo(Mask));
	Edge LOCALARRAY(result, graph.getEdgeNo(Mask));
	int cnt = 0;
	int lenEdges = 0;
	for(; beg!=end; ++beg)
		edges[lenEdges++] = *beg;
	...
	while(lenEdges>0) {
		locMax++;
		//we use matching to color edges
		int lenRes = getMatching(graph, edges, edges+lenEdges, result);
		assert(lenRes>0);
		int iEdges=0, iRes=0, cpEdges=0;
		while(cpEdges<lenEdges && iRes<lenRes) {
			if(iEdges!=cpEdges)
				edges[iEdges] = edges[cpEdges];
			if(edges[iEdges]==result[iRes])
				colors[ result[iRes++] ] = locMax;
			else
				++iEdges;
			++cpEdges;
		}
		while(cpEdges<lenEdges)
			edges[iEdges++] = edges[cpEdges++];
		lenEdges = iEdges;
	}
	return locMax;
